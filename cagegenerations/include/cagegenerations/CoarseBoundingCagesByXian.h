...
struct BBVoxel_ {
    ...
    {
        return z + y * n_voxel[2] + x * n_voxel[2] * n_voxel[1];
    }

    int getCenterId(int x, int y, int z)
    {
        return z + y * n_voxel[2] + x * n_voxel[2] * n_voxel[1];
    }

    int getVoxelPtsId(int x, int y, int z)
    {
        return z + y * (n_voxel[2]+1) + x * (n_voxel[2]+1) * (n_voxel[1]+1);
    }
};
...
void voxelizeBB(const Eigen::MatrixXd& mesh_vertices, const Eigen::MatrixXd& cage_vertices, BBVoxel& voxels, BBVoxel_& voxels_, float degreeOfSparseness);
